...
export default function EditCardScreen() {
  ...
  useEffect(() => {
    if (card) {
      ...
      setBarcodeType(card.barcodeType);
      setColor(card.color as CardGradientKey);
      setNotes(card.notes || '');
    }
  }, [card]);

  // Update state when scan params change
  useEffect(() => {
  ...
  const handleScan = useCallback(() => {
    ...
    router.push({
      pathname: '/scan',
      params: { returnTo: 'edit-card', cardId: params.id },
    });
  }, [router, params.id]);
      ...
      <KeyboardAvoidingView
        ...
        <ScrollView
          ...
          <Input
            ...
            placeholder={t('cardForm.cardNumberPlaceholderEdit')}
            value={cardNumber}
